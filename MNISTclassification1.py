...
X_test = (X_test - min_train) / (max_train - min_train) #  Normalize X_test based on the maximum value of X_trainrain
...
for s in Min_sample:
  model_RF = RandomForestClassifier(min_samples_leaf=s, random_state=12) # define Random forest model
...
plt.ylabel("Accuracy")
plt.title("KNN Accuracy vs Number of Neighbors MNIST 1")
plt.legend()
plt.grid()
plt.show()
...
plt.ylabel("Accuracy")
plt.title("Random Forest Accuracy vs Min Sample Leaf MNIST 1")
plt.legend()
plt.grid()
plt.show()
